...
const AddTrack = () => {
    ...
    const [statusText, setStatusText] = useState("Выберите статус | 選擇狀態");
    ...
    const handleStatusClick = (statusId, statusText) => {
        ...
        const translatedStatusText = 
            statusText === "Поступило на склад в Китае"
                ? `${statusText} (中國倉庫收到)` 
                : statusText === "Выехало со склада в Китае"
                ? `${statusText} (從中國倉庫出貨)` 
                : statusText;
        
        setStatus(statusId);
        setStatusText(translatedStatusText);
                ...
                <div className="mini-contianer">
                    <div className="status-excel-container">
                        <div className="date-container_mobile">
                            <h3 className="h3-date">Выберите дату | 選擇日期</h3>
                            <input 
                                type="date" 
                        ...
                        <div className="statuses">
                            ...
                            {role !== 'filial' && isPopupOpen && (
                                statuses
                                    ...
                                    .map(status => (
                                        <div key={status._id} className="status-el status-pop" onClick={() => handleStatusClick(status._id, status.statusText)} >
                                            <p>{status.statusText === "Поступило на склад в Китае"
                                                ? `${status.statusText} (中國倉庫收到)`
                                                : status.statusText === "Выехало со склада в Китае"
                                                ? `${status.statusText} (從中國倉庫離開)`
                                                : status.statusText
                                            }</p>
                                        </div>
                                    ))
                        ...
                        <div className="excel-block" onClick={handleOpenModal}>
                            <p>Массовая загрузка | 載入曲目</p>
                            <img src={tableList} alt="" />
                        </div>
                        {modalOpen && (
                            <div className="modalExcel">
                                <div className="modal-header">
                                    <h2>Массовая загрузка | 載入曲目</h2>
